...
import utils.utils_black_scholes as bs
...
from makers.kandel import Kandel
...
i_nb_day = col1up.number_input("Nb simulated days", 1, 750, 8, step = 1)
i_nb_step_day = col1up.slider("Nb intraday simulations", 0, 10, 1)
i_seed = col1up.number_input("Seed for diffusion", 0, None, 123)

i_yield = col2up.slider("Yearly return (%, μ)", -10, 50, 0) / 100
i_volat = col2up.slider("Yearly volatility (%, σ)", 1, 150, 60) / 100
i_px_init = col2up.number_input("Initial price (S_0)", 50.0, 20000.0, 2000.0, step=1.0)

i_time_delta = 1 / (float(NB_DAY_PER_YEAR) * float(i_nb_step_day))


def simulate_path(seed : int = None,
                return_seed : bool = False):
    if not seed:
        seed = int(int.from_bytes(os.urandom(3), byteorder="little"))
    path =  geom_brownian_path(
        i_px_init, i_yield, i_volat, i_nb_day,
         i_nb_step_day, NB_DAY_PER_YEAR, seed, 
         price_to_start = False)
    
    if return_seed : 
        return path, seed, np.std(np.log(path[1:] / path[:-1])) * np.sqrt(i_nb_step_day) * np.sqrt(NB_DAY_PER_YEAR)
    return path
...
i_strike = col1up.number_input("Option Strike K", 0.0, i_px_init * 2.0, i_px_init, step=1.0)
...
#     step=10,
# )
i_volat_repli = col2up.number_input("Volatility used for replication (%, σ)", 1.0, 200.0, 75.0, step = 1.0) / 100
...
def time_clock():
    ...
    st.session_state[sTimeOver] = time_idx >= len(i_prices) - 1
...
def monte_carlo_one(resStore: List, seed : int = None):

    maker = build_maker(use_latest_price=False)
    exchange = ExchangeSingleMaker(maker)

    path, seed, realized_vol = simulate_path(seed = seed, return_seed=True)
    time = 0
    dt = i_time_delta
    for price in path:
        time += dt
        ...
        resStore.append(
            [
                state_context_repr(),
                price,
                exchange.maker.cash - i_call_price,
...
def display_monte_carlo():
    sims = pd.DataFrame(
        state_get(sMonteCarlo[-1]), columns=["maker", "price", "cash", 
                                         "asset", "nb_tx", "seed", "realized_vol"]
    )
    ...
    with placeholder.container():
        ...
        sims = sims[["price", "pnl", "maker", "seed", "realized_vol"]]
        sims["tag"] = sims.realized_vol > i_volat
        
        pnl_mean = sims.pnl.mean()
        pnl_std = sims.pnl.std()
        ...
        fig = px.scatter(sims, x="price", y="pnl", 
                         color="tag", hover_name = "seed",
                         hover_data = ["price", "pnl", "realized_vol"]).add_hline(y=0)
        #if i_add_call:
        #    fig.add_hline(y=i_call_price, name="call price", opacity=0.3)
        fig.add_hline(y=pnl_mean, name="mean_pnl", opacity=0.3, 
                      line_color = "blue", annotation_text = f'Avg: {pnl_mean:.2f}')
        fig.add_hrect(y0=pnl_mean - pnl_std, y1=pnl_mean + pnl_std, line_color = "blue", 
                      opacity = 0.3, annotation_text=f'Std Dev: {pnl_std:.2f}')
        fig.add_annotation(text = f"# Simuls : {len(sims)}",
...
if rightSide.button("One Simulation"):
    state_set(sMonteCarloCancel, False)
    monte_carlo_n(1)
    display_monte_carlo()

if rightSide.button("Simulate Input Seed"):
    state_set(sMonteCarloCancel, False)
    resStore = state_get(sMonteCarlo)
    monte_carlo_one(resStore, seed = i_seed)
    display_monte_carlo()
