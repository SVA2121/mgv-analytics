...
WITH RECURSIVE block_series AS (
    ...
    SELECT block + 1
    FROM block_series
    WHERE block < (SELECT MAX(LOWER(block_range)) FROM sgd83.order)
)
, prices AS (
  SELECT DISTINCT
    ...
    FROM sgd83.order AS o
    LEFT JOIN sgd83.market AS market
        ON o.market = market.id
    LEFT JOIN sgd83.token AS out_token
      ON market.outbound_tkn = out_token.id
     LEFT JOIN sgd83.token AS in_token
